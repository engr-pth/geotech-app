...
# ----------------------------------------------------
    ...
    st.title("🏗️ Bored Pile Design")

    st.set_page_config(page_title="Bored Pile Capacity Calculator", layout="wide")
...
# ==========================================
st.header(
    "1. General, Material & Pile Parameters (အထွေထွေနှင့် ပိုင်တိုင်"
    " ကန့်သတ်ချက်များ)"
)
...
with col_p1:
  pile_diameter = st.number_input(
      "Pile Diameter (m) - ပိုင်တိုင် အချင်း",
      value=0.50,
      step=0.05,
      on_change=reset_calc,
  )
with col_p2:
  fs_factor = st.number_input(
      "Geotech FS - ဘေးကင်းကိန်း",
      value=2.00,
      step=0.10,
      on_change=reset_calc,
  )
with col_p3:
  target_capacity = st.number_input(
      "Target Q_allow (ton) - လိုအပ်သော ဝန်ထမ်းအား",
      value=100.00,
      step=10.0,
      on_change=reset_calc,
  )
with col_p4:
  step = st.number_input(
      "Calculation Step (m) - တွက်ချက်မည့် အလွှာအထူ အတိုင်းအတာ",
      value=0.50,
      step=0.1,
      on_change=reset_calc,
  )
with col_p5:
  gwt_depth = st.number_input(
      "GWT Depth (m) - ရေမျက်နှာပြင် အနက်",
      value=2.00,
      step=0.5,
      min_value=0.0,
      on_change=reset_calc,
...
st.subheader(
    "RC Structural Parameters & Standard Code (ကွန်ကရစ် တည်ဆောက်ပုံဆိုင်ရာ"
    " ကန့်သတ်ချက်များ)"
)
col_s1, col_s2, col_s3, col_s4, col_s5 = st.columns(5)
with col_s1:
  aci_version = st.selectbox(
      "Structural Code - ဒီဇိုင်းကုဒ်",
      ["ACI 318-19", "ACI 318-14", "ACI 318-11"],
      index=0,
      on_change=reset_calc,
  )
with col_s2:
  fc_prime = st.number_input(
      "Concrete f'c (MPa) - ကွန်ကရစ် ခိုင်ခံ့အား",
      value=25.0,
      step=1.0,
      on_change=reset_calc,
  )
with col_s3:
  fy_rebar = st.number_input(
      "Steel fy (MPa) - သံချောင်း ခိုင်ခံ့အား",
      value=400.0,
      step=10.0,
      on_change=reset_calc,
  )
with col_s4:
  rebar_ratio_pct = st.number_input(
      "Steel Ratio ρ (%) - သံချောင်းပါဝင်မှု ရာခိုင်နှုန်း",
      value=1.0,
      step=0.1,
      min_value=0.5,
      max_value=4.0,
...
with col_s5:
  phi_structural = st.number_input(
      "Strength Factor (ϕ) - လျှော့ချကိန်း",
      value=0.75,
      step=0.05,
      help="Spiral = 0.75, Tied = 0.65",
      on_change=reset_calc,
  )
...
# Custom CSS with strict soil-scroll-wrapper for Mobile Horizontal Scrolling
st.markdown(
    """
<style>
...
# Closing soil-scroll-wrapper container
...
with b_col1:
  if st.button(
      "➕ Add Soil Layer (မြေအလွှာ အသစ်ထပ်ရန်)", use_container_width=True
  ):
...
total_thickness = sum(
    [float(l["Thickness"]) for l in st.session_state.soil_layers]
)
st.metric(
...
# ==========================================
# DESIGN BUTTON (STRICTLY CONTROLS CALCULATION)
# ==========================================
if st.button(
    "🚀 Design Pile (ပိုင်တိုင် ဒီဇိုင်း စတင်တွက်မည်)",
    type="primary",
...
# ==========================================
# 3. CALCULATION & REPORT DISPLAY (ONLY SHOWN WHEN CALCULATED IS TRUE)
# ==========================================
if st.session_state.calculated and len(st.session_state.soil_layers) > 0:
  ...
  K_alpha = 0.85
  P_n_max_kN = K_alpha * (
      0.85 * (fc_prime * 1e3) * A_c + (fy_rebar * 1e3) * A_st
  )
  ...
  sc2.metric(
      "Steel Area (Ast)", f"{(A_st * 1e4):.1f} cm² ({rebar_ratio_pct}%)"
  )
  sc3.metric(
      f"Structural Capacity ({aci_version})",
      f"{P_design_comp_ton:.1f} ton",
      help=f"Design strength ϕPn per {aci_version}",
  )
  ...
  for d in depths:
      ...
      if d > layer_top:
        ...
        if liq:
          ...
          if l_type == "cohesive":
            cu = 6.25 * n_spt
            alpha = (
                0.55 if cu <= 60 else max(0.35, 0.55 - 0.005 * (cu - 60))
            )
            f_s = alpha * cu
          else:
    ...
    tip_layer = (
        ...
        else {"Type": "cohesionless", "N_SPT": 10, "Liquefiable": False}
    )
    current_depth = 0.0
    for layer in soil_profile:
    ...
    if bool(tip_layer["Liquefiable"]):
      ...
      if str(tip_layer["Type"]) == "cohesive":
        ...
        q_b_spt = 40.0 * tip_n
        q_b_eff = (
            3.0 * tip_n * eff_stress_tip / 100.0
            if eff_stress_tip > 0
            else q_b_spt
        )
    ...
    if (
        req_depth is None
        and qa_ton >= target_capacity
        and qa_ton <= struct_allow_service_ton
    ):
      req_depth = d
  ...
  # ==========================================
  # 4. MATPLOTLIB REPORT GENERATION (FIXED WIDTH RATIOS & SPACING)
  # ==========================================
  ...
  ax1.plot(
      ...
      "b-",
      linewidth=2.5,
      label="Cumulative Shaft Res. ($Q_s$)",
  )
  ax1.plot(
      df_res["Qb (ton)"],
      df_res["Depth (m)"],
  ...
  ax2.plot(
      df_res["Q_allow Geotech (ton)"],
      df_res["Depth (m)"],
      "g-",
      linewidth=2.5,
      label=f"Geotech $Q_{{allow}}$ (FS={fs_factor})",
  )
  ax2.axvline(
      x=struct_allow_service_ton,
      color="darkred",
      linestyle="-.",
      linewidth=2.2,
      label=f"Struct. Allowable Limit",
  )
  ax2.axvline(
      x=target_capacity,
      color="purple",
      linestyle="--",
      linewidth=2.0,
      label=f"Target $Q_{{allow}}$ ({target_capacity}t)",
  )
  ...
  if req_depth is not None:
    ...
    ax2.annotate(
        ...
        f" {target_capacity:.1f} ton",
        xy=(target_capacity, req_depth),
        xytext=(
            max(10.0, target_capacity - 60),
            min(max_depth - 1.5, req_depth + 3),
        ),
        bbox=dict(
            boxstyle="round,pad=0.5", fc="yellow", ec="orange", lw=1.5
        ),
        arrowprops=dict(

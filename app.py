...
def generate_bored_pile_pdf(fig, df_res, pile_diameter, fs_factor, target_capacity, aci_version, fc_prime, fy_rebar, rebar_ratio_pct, P_design_comp_ton, req_depth, soil_profile):
    buffer = io.BytesIO()
    ...
    subtitle_style = ParagraphStyle('SubTitleStyle', parent=styles['Normal'], fontSize=9, leading=12, alignment=1, textColor=colors.HexColor('#4a5568'))
    h2_style = ParagraphStyle('H2Style', parent=styles['Heading2'], fontSize=12, leading=16, textColor=colors.HexColor('#2b6cb0'), spaceBefore=10, spaceAfter=5)
    body_style = ParagraphStyle('BodyStyle', parent=styles['Normal'], fontSize=9, leading=13, textColor=colors.HexColor('#2d3748'))
    formula_style = ParagraphStyle('FormulaStyle', parent=styles['Normal'], fontSize=8.5, leading=12, textColor=colors.HexColor('#1a202c'), backColor=colors.HexColor('#f7fafc'), borderPadding=6)

    # Title Section
    story.append(Paragraph("BORED PILE CAPACITY & GOVERNING DESIGN REPORT", title_style))
    story.append(Paragraph("Designed & Developed by: Engr. Phyo Thi Han, BE(Civil), ME(Civil Geotechnical), RE", subtitle_style))
    story.append(Spacer(1, 10))

    # General Input Parameters Table
    story.append(Paragraph("1. Design Input Parameters", h2_style))
    input_data = [
        ["Parameter", "Value", "Parameter", "Value"],
        ["Pile Diameter (D)", f"{pile_diameter} m", "Concrete Strength (f'c)", f"{fc_prime} MPa"],
        ["Safety Factor (FS)", f"{fs_factor}", "Steel Yield (fy)", f"{fy_rebar} MPa"],
        ["Target Capacity", f"{target_capacity} ton", "Steel Ratio (ρ)", f"{rebar_ratio_pct} %"],
        ["Design Code", f"{aci_version}", "Struct. Allowable Cap.", f"{(P_design_comp_ton/1.4):.1f} ton"]
    ]
    t_input = Table(input_data, colWidths=[130, 120, 130, 120])
    ...
    story.append(Paragraph("2. Calculation Methodology & Equations (တွက်နည်းအဆင့်ဆင့်)", h2_style))
    method_text = f"""
    <b>A. Structural Axial Capacity ({aci_version}):</b><br/>
    • Nominal Capacity: <i>P_n = 0.85 × [0.85 · f'c · (A_g - A_st) + f_y · A_st]</i><br/>
    • Design Structural Capacity: <i>P_design = ϕ · P_n</i> (Where ϕ = 0.75)<br/>
    • Allowable Structural Capacity: <i>P_struct,allow = P_design / 1.4</i><br/><br/>
    <b>B. Geotechnical Ultimate Capacity (Meyerhof / Tomlinson Method):</b><br/>
    • Ultimate Capacity: <i>Q_u = Q_s + Q_b</i> | Allowable: <i>Q_allow = Q_u / FS</i><br/>
    • <b>Cohesive Soil (Clay):</b><br/>
    &nbsp;&nbsp;- Undrained Shear Strength: <i>c_u = 6.25 × N_SPT</i> (kPa)<br/>
    &nbsp;&nbsp;- Shaft Resistance: <i>f_s = α × c_u</i> (Max 150 kPa), where α = 0.55 for c_u ≤ 60 kPa<br/>
    &nbsp;&nbsp;- End Bearing: <i>q_b = 9 × c_u</i> (Max 5000 kPa)<br/>
    • <b>Cohesionless Soil (Sand):</b><br/>
    &nbsp;&nbsp;- Shaft Resistance: <i>f_s = min(2 × N_SPT, β × σ'_v)</i> (Max 100 kPa), where <i>β = 1.5 - 0.245√z</i><br/>
    &nbsp;&nbsp;- End Bearing: <i>q_b = min(40 × N_SPT, 3 × N_SPT × σ'_v / 100)</i> (Max 4000 kPa)<br/><br/>
    <b>C. Governing Criteria:</b><br/>
    • <i>Q_final = min(Q_allow,geotech , P_struct,allow)</i>
    """
    story.append(Paragraph(method_text, formula_style))
    story.append(Spacer(1, 10))

    # Save Chart Image & Add to PDF
    img_buf = io.BytesIO()
    fig.savefig(img_buf, format='png', dpi=200, bbox_inches='tight')
    img_buf.seek(0)
    story.append(Paragraph("3. Soil Profile & Capacity Curves", h2_style))
    story.append(RLImage(img_buf, width=520, height=210))
    story.append(Spacer(1, 10))

    # Results Table
    story.append(Paragraph("4. Depth-wise Capacity Summary", h2_style))
    res_headers = [["Depth (m)", "Eff. Stress", "Qs (ton)", "Qb (ton)", "Qu (ton)", "Q_allow (ton)", "Governing Mode"]]
    res_data = res_headers + df_res[["Depth (m)", "Eff. Stress (kPa)", "Cum. Qs (ton)", "Qb (ton)", "Qu Geotech (ton)", "Q_allow Geotech (ton)", "Governing Mode"]].values.tolist()
    
    t_res = Table(res_data, colWidths=[60, 70, 70, 60, 70, 85, 85])
...
# 5. BORED PILE PAGE
    ...
    if st.session_state.calculated and len(st.session_state.soil_layers) > 0:
        ...
        pdf_data = generate_bored_pile_pdf(
            fig, df_res, pile_diameter, fs_factor, target_capacity, aci_version,
            fc_prime, fy_rebar, rebar_ratio_pct, P_design_comp_ton, req_depth, soil_profile
        )
        
        st.download_button(
            label="📄 Download Detailed PDF Calculation Report",
            data=pdf_data,
            file_name=f"Bored_Pile_Design_Report_{pile_diameter}m.pdf",
            mime="application/pdf",
            use_container_width=True
        )
